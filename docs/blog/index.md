---
layout: default
title: 欢迎来到糖友空间博客
description: "您好, 欢迎访问 **糖友空间博客**。本站是 **糖友空间** 的官方内容中心, 致力于成为您在糖尿病管理路上的可靠伙伴。"
author: "张杨 (Dane Zhang)"
author_title: "健康科技研究员"
lang: zh
translation_key: blog
---
# 欢迎来到糖友空间博客

> **[English Version](./index-en)** | **[英文版入口](./index-en)**

您好, 欢迎访问 **[糖友空间博客](https://www.tangyou.space/blog/)**。本站是 **[糖友空间](https://shop.tangyou.space)** 的官方内容中心, 致力于成为您在糖尿病管理路上的可靠伙伴。

我们的核心使命是分享与糖尿病相关的知识, 全面覆盖从 **预防**、**治疗** 到 **康复** 的每一个关键阶段。

---

## 糖尿病基础知识 (必读)

*   **[警惕“甜蜜的杀手”：糖尿病并发症全景概述](./diabetes-complications-overview)**
*   **[全球主流糖尿病诊断标准参考](./diabetes-diagnostic-criteria)**

## 生活与管理 (最新更新)

*   **[糖尿病A到Z基础入门：一份实用起步指南](./prevention/sleep/diabetes-a-to-z-basics)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[糖尿病前期预警信号：这个月先盯住哪些指标](./prevention/exercise/prediabetes-early-warning-signs)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[糖尿病关键术语速通：把看不懂变成看得懂](./rehabilitation/emotion/diabetes-terms-you-should-know)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[家庭糖尿病管理框架：能长期坚持的实用版本](./treatment/diet/home-diabetes-management-framework)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[1型糖尿病：把门诊建议落成家庭日程](./treatment/emotion/type1-diabetes-clinic-to-home-plan)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[2型糖尿病监测协议：家庭可执行版本](./treatment/exercise/type2-diabetes-monitoring-protocols)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[糖尿病随访中的内分泌检查：重点看什么](./treatment/sleep/endocrinology-tests-for-diabetes-followup)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[糖尿病高频问题实用解答](./prevention/emotion/diabetes-common-questions-answered)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[糖尿病预防与治疗路线图：一步一步做对](./rehabilitation/diet/diabetes-prevention-treatment-roadmap)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[低GI烹饪实战：忙碌家庭也能坚持的控糖餐](./prevention/diet/low-glycemic-cooking-practical-guide)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[沉默的伙伴：糖尿病亲友支持指南](./caregiver_support)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[驾驭护理成本：2026年糖尿病费用管理实用建议](./managing_diabetes_costs)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[关于糖尿病饮食规划的5个误区，你该停止相信了](./meal_planning_myths)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>
*   **[超越减肥：理解肥胖与2型糖尿病之间的生物学联系](./obesity_diabetes_link)** <span style="font-size: 0.8em; background-color: #ffcccc; padding: 2px 5px; border-radius: 3px;">New</span>

---

## 核心关注：糖尿病管理三部曲

### 1. 预防阶段

*   [进入 **预防** 分类](./prevention/)

### 2. 治疗阶段

*   [进入 **治疗** 分类](./treatment/)

### 3. 康复阶段

*   [进入 **康复** 分类](./rehabilitation/)

---

## 增长与变现

*   [进入 **网络广告联盟** 分类](./affiliate/)
*   [进入 **SEO** 分类](./seo/)

---

## 个人记录与思考

*   **[关于我](./aboutme)**
*   **[个人健康日志](./health-log/)**
*   **[技术个人思考](./tech-thoughts/)**
