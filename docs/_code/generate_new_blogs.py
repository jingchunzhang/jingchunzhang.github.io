...
4. 情感真挚，避免过于机械""",
    ...
    "buying_guide": """请生成一篇"{keyword}"的选购指南。要求：
1. 明确目标人群和使用场景
2. 列出选购要点（功能、价格、品牌等）
3. 给出推荐产品或方案"""
}
...
def create_front_matter(blog: dict, author_info: dict) -> str:
    ...
    if blog['is_english']:
        ...
        description = f"{blog['keyword']} - Comprehensive guide"
    else:
        title = blog['keyword']
        description = f"{blog['keyword']} - 全面指南"
...
def main():
    with open('_code/temp/new_blog_tasks.json', 'r', encoding='utf-8') as f:
        blogs = json.load(f)
    
    print(f"开始生成 {len(blogs)} 篇全新博客...")
